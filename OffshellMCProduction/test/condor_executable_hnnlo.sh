...
# We need to require SL7 for Run 2 legacy reco. to work.
# If the machine uses SL6, we have to use singularity.
# This is often the case in submissions from lxplus.
MACHINESPECS="$(uname -a)"
echo "Machine specifics: ${MACHINESPECS}"
declare -i FOUND_EL7=0
if [[ "${MACHINESPECS}" == *"el7"* ]]; then
  FOUND_EL7=1
else
  for evar in $(env); do
    if [[ "$evar" == *"SINGULARITY_IMAGE_HUMAN"* ]]; then
      if [[ "$evar" == *"rhel7"* ]] || [[ "$evar" == *"slc7"* ]]; then
        FOUND_EL7=1
      fi
    fi
  done
...
if [[ ${FOUND_EL7} -eq 1 ]] && [[ ${HAS_CMSSW} -eq 0 ]]; then
  ...
  echo "Machine does not have the proper setup to run native calls. Singularity with an SL7 docker container will be used."
fi
...
SINGULARITYCONTAINER="docker://cmssw/slc7:latest"
