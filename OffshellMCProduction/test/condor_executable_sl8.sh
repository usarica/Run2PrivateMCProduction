...
declare -i FOUND_EL_VERSION=0
if [[ "${MACHINESPECS}" == *"el8"* ]]; then
  FOUND_EL_VERSION=1
else
  for evar in $(env); do
...
if [[ ${FOUND_EL_VERSION} -eq 1 ]] && [[ ${HAS_CMSSW} -eq 0 ]]; then
  USE_NATIVE_CALLS=1
  echo "Machine has both CMS environment scripts and runs on the required SL architecture. Native calls will be used."
else
  echo "Machine does not have the proper setup to run native calls. Singularity with a docker container will be used."
fi
...
if [[ -e "/cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el8:amd64" ]]; then
  SINGULARITYCONTAINER="/cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el8:amd64"
elif [[ -e "/cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el8:x86_64" ]]; then
  SINGULARITYCONTAINER="/cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el8:x86_64"
elif [[ $USE_NATIVE_CALLS -eq 0 ]]; then
  echo "ERROR: Singularity container is not found."
  exit 1
fi
