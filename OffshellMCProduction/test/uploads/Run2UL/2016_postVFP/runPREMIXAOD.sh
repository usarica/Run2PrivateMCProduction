...
if [[ ! -e premixraw_cfg.py ]]; then
  ...
  cmsDriver.py --python_filename premixraw_cfg.py \
    ...
    --conditions 106X_mcRun2_asymptotic_v13 --step DIGI,DATAMIX,L1,DIGI2RAW --procModifiers premix_stage2 --geometry DB:Extended \
    --datamix PreMix --era Run2_2016 --runUnscheduled \
    --no_exec --mc -n -1 || exit $?
fi
...
if [[ ! -e aod_cfg.py ]]; then
  ...
  cmsDriver.py --python_filename aod_cfg.py \
    ...
    --customise Configuration/DataProcessing/Utils.addMonitoring \
    --conditions 106X_mcRun2_asymptotic_v13 --step RAW2DIGI,L1Reco,RECO,RECOSIM --geometry DB:Extended --era Run2_2016 \
