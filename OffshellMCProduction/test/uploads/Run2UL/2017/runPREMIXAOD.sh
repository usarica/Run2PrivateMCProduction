...
if [[ ! -e premixraw_cfg.py ]]; then
  ...
  cmsDriver.py --python_filename premixraw_cfg.py \
    ...
    --conditions 106X_mc2017_realistic_v6 --step DIGI,DATAMIX,L1,DIGI2RAW --procModifiers premix_stage2 --geometry DB:Extended \
    --datamix PreMix --era Run2_2017 --runUnscheduled \
    --no_exec --mc -n -1 || exit $?
fi
...
cmssw_version=CMSSW_9_4_14_UL_patch1
export SCRAM_ARCH=${scram_arch}
source /cvmfs/cms.cern.ch/cmsset_default.sh
if [[ -r ${cmssw_version}/src ]]; then
  echo release ${cmssw_version} already exists
...
if [[ ! -e hlt_cfg.py ]]; then
  ...
  cmsDriver.py --python_filename hlt_cfg.py \
    ...
    --customise Configuration/DataProcessing/Utils.addMonitoring \
    --conditions 94X_mc2017_realistic_v15 --step HLT:2e34v40 --geometry DB:Extended --era Run2_2017 \
...
if [[ ! -e aod_cfg.py ]]; then
  ...
  cmsDriver.py --python_filename aod_cfg.py \
    ...
    --customise Configuration/DataProcessing/Utils.addMonitoring \
    --conditions 106X_mc2017_realistic_v6 --step RAW2DIGI,L1Reco,RECO,RECOSIM --geometry DB:Extended --era Run2_2017 \
